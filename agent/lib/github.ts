 ...
 */
/** Selection helpers live in ./select so the browser never imports this server-only module. */
export { changedLines, selectReviewFiles } from "./select";
...
const PR_URL = new RegExp(`^https?:\\/\\/(?:www\\.)?github\\.com\\/(${NAME})\\/(${NAME})\\/pull\\/(\\d+)(?:[/?#].*)?$`);

export function parsePullRequestUrl(input: string): { owner: string; repo: string; number: number } | null {
...
export async function fetchPullRequest(input: string, token = process.env.GITHUB_TOKEN): Promise<PullRequestReview> {
  const ref = parsePullRequestUrl(input);
  if (!ref) throw new Error("That is not a GitHub pull request URL (expected github.com/owner/repo/pull/123).");
  ...
  if (meta.status === 404) await fail("Pull request not found. Private repositories are not supported.");
  if (meta.status === 403 || meta.status === 429) await fail("GitHub rate limit reached. Try again in a few minutes.");
  if (!meta.ok) await fail(`GitHub returned ${meta.status} for the pull request.`);
  if (!diff.ok) await fail(`GitHub returned ${diff.status} for the diff.`);
  const length = Number(diff.headers.get("content-length") ?? 0);
  if (length > MAX_DIFF_BYTES) await fail("That pull request's diff is too large to judge here.");
  const json = (await meta.json()) as { title?: string; body?: string | null; html_url?: string; changed_files?: number };
  const text = await readCapped(diff, MAX_DIFF_BYTES);
  if (text === null) throw new Error("That pull request's diff is too large to judge here.");
  return {
