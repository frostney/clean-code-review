...
export function switchView(update: () => void): void {
  ...
  transition.ready.catch(() => {
    /* A transition the browser skipped — a hidden tab, a second one started
       on top of this one — rejects here. The page has changed either way,
       and an animation nobody saw is not something to report. */
  });
}
