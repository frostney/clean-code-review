...
${SITE.name} judges code against the chapters of Robert C. Martin's *Clean Code*. Point it at a public GitHub pull request, or paste a unified diff or a file, and Jev — TypeSafe's evaluation model, reached through the Vercel AI Gateway — answers ${QUESTION_COUNT} questions about every file in the change in a single turn: names, functions, comments, formatting, objects, error handling, tests, classes and smells. Every answer is a probability, a score or a choice rather than prose, so the verdict is made of things a reader can check. Luna then writes the review from those findings, without ever seeing the code. Every example on the page is editable, and an edit re-judges only the file that changed. The code is held in the browser tab's own eve session for the turn it is judged in and is not stored afterwards; there is no account and nothing to sign in to.
